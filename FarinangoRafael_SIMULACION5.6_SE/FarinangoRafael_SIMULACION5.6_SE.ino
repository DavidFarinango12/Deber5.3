/*
  ...
  usart
  all
  https://www.nongnu.org/avr-libc/usermanual/group__avr__power.html
*/
...
void setup() {
  ...
  MsTimer2::set(1000, reloj); // configura timer2 a un segundo
...
void loop() {
  Serial.println(analogRead(A0)); // confirmación de desactivación del cad
  delay(1000);
}
...
void configuracion() { // rutina interrupcion 1
  if (on == 1) {
    //Serial.println("on vale = "+String(on));
    tiempo = tiempo + 10;
    if (tiempo < 110) {
...
void reloj() { // rutina de timer 2
  ...
  Serial.println(String("Lectura en: ") + String(cont)); // mensaje de tiempo
  if (cont == tiempo) { // comparación con selección de usuario
    power_adc_enable(); // habilita adc
    delay(10);
    Serial.println(analogRead(A0)); // realiza la lectura
